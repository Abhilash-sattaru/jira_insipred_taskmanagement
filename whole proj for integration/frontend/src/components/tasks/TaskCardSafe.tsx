...
const TaskCardSafe: React.FC<TaskCardProps> = ({
  task,
  isDragging,
  onClick,
}) => {
  ...
  const loadEmployees = React.useCallback(async () => {
    ...
    try {
      ...
      console.error("TaskCardSafe: failed to load employees", err);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {loadError && (
          <div className="mb-2 p-2 rounded bg-destructive/10 text-destructive text-xs flex items-center justify-between">
            <div className="truncate">
              Employee names blocked by server: {loadError}
            </div>
            <button
